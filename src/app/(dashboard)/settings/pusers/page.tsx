    ...
    <Card className="rounded-xl border shadow-sm bg-white dark:bg-slate-900 overflow-hidden">
      <CardHeader className="p-8 bg-slate-50 dark:bg-slate-900/50 border-b shrink-0">
        <div className="flex items-center gap-6">
          ...
          <div>
            <CardTitle className="text-xl font-headline font-bold uppercase tracking-tight text-slate-900 dark:text-white">Plattform Administratoren</CardTitle>
      ...
      <CardContent className="p-0">
        <Table>
          <TableHeader className="bg-slate-50/30 dark:bg-slate-950/30">
            <TableRow className="hover:bg-transparent border-slate-100 dark:border-slate-800">
              <TableHead className="py-5 px-8 font-black uppercase tracking-[0.1em] text-[10px] text-slate-400">Administrator</TableHead>
              <TableHead className="font-black uppercase tracking-[0.1em] text-[10px] text-slate-400">Rolle</TableHead>
              <TableHead className="font-black uppercase tracking-[0.1em] text-[10px] text-slate-400">Zuständigkeit</TableHead>
              <TableHead className="text-right px-8 font-black uppercase tracking-[0.1em] text-[10px] text-slate-400">Aktionen</TableHead>
            </TableRow>
          </TableHeader>
          <TableBody>
            {pUsers?.map(u => (
              <TableRow key={u.id} className="group hover:bg-slate-50 dark:hover:bg-slate-800/50 border-slate-100 dark:border-slate-800 transition-colors">
                <TableCell className="py-4 px-8">
                  <div className="flex items-center gap-4">
                    <div className="w-9 h-9 rounded-lg bg-slate-100 dark:bg-slate-800 flex items-center justify-center text-primary font-black text-xs border border-slate-200 dark:border-slate-700 shadow-inner">
                      {u.displayName?.charAt(0) || 'A'}
                    </div>
                    <div>
                      <div className="font-bold text-sm text-slate-800 dark:text-slate-100">{u.displayName}</div>
                      <div className="text-[9px] text-slate-400 font-medium flex items-center gap-1.5 mt-0.5">
                ...
                <TableCell>
                  <Badge variant="outline" className="rounded-full text-[8px] font-black uppercase px-3 h-5 border-slate-200 dark:border-slate-800 text-slate-500">
                    {u.role}
                  </Badge>
                </TableCell>
                <TableCell>
                ...
                <TableCell className="text-right px-8">
                  <Button 
                    variant="ghost" 
                    size="icon" 
                    className="h-8 w-8 rounded-lg text-red-500 hover:bg-red-50 dark:hover:bg-red-900/20 opacity-0 group-hover:opacity-100 transition-all" 
        ...
        {(!pUsers || pUsers.length === 0) && (
          <div className="py-20 text-center space-y-4">
            ...
            <p className="text-[10px] font-black uppercase text-slate-400 tracking-[0.2em]">Keine Administratoren registriert</p>
          </div>
        )}
      </CardContent>
    </Card>
