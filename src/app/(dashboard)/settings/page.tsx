...
import { useSettings } from '@/context/settings-context'; // Importieren
import { RadioGroup, RadioGroupItem } from '@/components/ui/radio-group'; // Importieren

export default function SettingsPage() {
  const [tenantName, setTenantName] = useState('Acme Corp');
  const [tenantSlug, setTenantSlug] = useState('acme');
  const { dataSource, setDataSource } = useSettings(); // Hook verwenden
    ...
    <div className="space-y-8 animate-in fade-in duration-500 max-w-5xl mx-auto">
      ...
      <Tabs defaultValue="general" className="space-y-6">
        <TabsList className="bg-card border h-12 p-1 gap-1 rounded-xl">
          ...
          <TabsTrigger value="data" className="data-[state=active]:bg-primary data-[state=active]:text-primary-foreground rounded-lg px-6 gap-2"> {/* Neuer Tab */}
        ...
        <TabsContent value="data" className="space-y-6"> {/* Neuer Tab-Inhalt */}
