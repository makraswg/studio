  ...
  return (
    <div className="space-y-6 pb-10">
      <div className="flex flex-col md:flex-row md:items-end justify-between gap-4 border-b pb-6">
        ...
        <Button size="sm" className="h-9 rounded-md font-bold text-xs px-6 bg-primary hover:bg-primary/90 text-white shadow-lg active:scale-95" onClick={() => { resetForm(); setIsDialogOpen(true); }}>
      ...
      <div className="bg-white dark:bg-slate-900 rounded-xl border shadow-sm overflow-hidden">
        {isLoading ? (
          <div className="flex flex-col items-center justify-center py-20 gap-3">
            ...
            <p className="text-[10px] font-bold text-slate-400">Lade Aufgaben...</p>
          </div>
        ) : (
          <Table>
      ...
      <Dialog open={isDialogOpen} onOpenChange={setIsDialogOpen}>
        <DialogContent className="max-w-2xl w-[95vw] rounded-xl p-0 overflow-hidden flex flex-col border-none shadow-2xl bg-white">
          ...
          <ScrollArea className="flex-1 bg-white">
            <div className="p-8 space-y-8">
              <div className="space-y-2">
                <Label className="text-[10px] font-bold uppercase text-slate-400 ml-1">Bezeichnung / Titel</Label>
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                <div className="space-y-2">
                  <Label className="text-[10px] font-bold uppercase text-slate-400 ml-1">Zuständigkeit (Assignee)</Label>
                  <Select value={assigneeId} onValueChange={setAssigneeId}>
                    <SelectTrigger className="rounded-xl h-11 border-slate-200 bg-white"><SelectValue placeholder="Wählen..." /></SelectTrigger>
                    <SelectContent>
                      {pUsers?.map(u => <SelectItem key={u.id} value={u.id}>{u.displayName}</SelectItem>)}
                    </SelectContent>
                  </Select>
                </div>
                <div className="space-y-2">
                  <Label className="text-[10px] font-bold uppercase text-slate-400 ml-1">Deadline</Label>
                  <Input type="date" value={dueDate} onChange={e => setDueDate(e.target.value)} className="rounded-xl h-11 border-slate-200" />
                </div>
                <div className="space-y-2">
                  <Label className="text-[10px] font-bold uppercase text-slate-400 ml-1">Priorität</Label>
              ...
              <div className="space-y-2">
                <Label className="text-[10px] font-bold uppercase text-slate-400 ml-1">Detailbeschreibung</Label>
                <Textarea value={desc} onChange={e => setDesc(e.target.value)} className="rounded-2xl min-h-[120px] text-xs font-medium border-slate-200 bg-slate-50/30" placeholder="Anweisungen für den Verantwortlichen..." />
              </div>
            </div>
          </ScrollArea>

          <DialogFooter className="p-4 bg-slate-50 border-t shrink-0 flex flex-col-reverse sm:flex-row gap-2">
            <Button variant="ghost" onClick={() => setIsDialogOpen(false)} className="rounded-xl font-bold text-[10px] px-8 h-11 text-slate-400 hover:bg-white">Abbrechen</Button>
            <Button onClick={handleSave} disabled={isSaving || !title || !assigneeId} className="rounded-xl font-bold text-[10px] tracking-widest px-12 h-11 bg-primary hover:bg-primary/90 text-white shadow-lg gap-2">
              ...
              <div className="flex-1 flex flex-col min-h-0 overflow-hidden md:flex-row">
                  ...
                  <div className="p-8 space-y-10">
                    <section className="space-y-4">
                      ...
                      <div className="grid grid-cols-2 gap-4">
                        <div className="p-4 rounded-2xl bg-slate-50 border border-slate-100 space-y-1">
                          <p className="text-[8px] font-black uppercase text-slate-400">Verantwortlich</p>
                        ...
                        <div className="p-4 rounded-2xl bg-slate-50 border border-slate-100 space-y-1">
                          <p className="text-[8px] font-black uppercase text-slate-400">Fälligkeit</p>
                          <div className="flex items-center gap-2 text-xs font-bold text-slate-900">
                      ...
                      <div className="p-6 bg-slate-50 rounded-2xl border border-slate-100">
                        ...
                        <p className="text-sm text-slate-700 leading-relaxed font-medium whitespace-pre-wrap">{selectedTask.description || 'Keine weitere Beschreibung hinterlegt.'}</p>
                      </div>
                    </section>
                    ...
                    {selectedTask.entityId && (
                      <section className="space-y-4">
                        ...
                        <div className="p-4 rounded-2xl bg-indigo-50/30 border border-indigo-100 flex items-center justify-between group">
                          <div>
                            <p className="text-[10px] font-bold text-indigo-900 uppercase">{selectedTask.entityType}</p>
                            <p className="text-[9px] text-indigo-400 font-bold mt-0.5">ID: {selectedTask.entityId}</p>
                          </div>
                          <Button variant="ghost" size="icon" className="h-8 w-8 text-indigo-600" onClick={() => router.push(`/${selectedTask.entityType === 'feature' ? 'features' : selectedTask.entityType === 'process' ? 'processhub' : selectedTask.entityType}s?search=${selectedTask.entityId}`)}>
                ...
                <aside className="w-full md:w-80 bg-slate-50 flex flex-col overflow-hidden">
                  <div className="p-4 border-b bg-white flex items-center justify-between shrink-0">
                    ...
                    <Badge variant="outline" className="text-[8px] font-bold rounded-full h-4 px-1.5">{comments?.filter(c => c.taskId === selectedTask.id).length || 0}</Badge>
                  </div>
                    ...
                    <div className="p-4 space-y-4">
                      {comments?.filter(c => c.taskId === selectedTask.id).sort((a,b) => new Date(b.createdAt).getTime() - new Date(a.createdAt).getTime()).map(c => (
                        <div key={c.id} className="space-y-1">
                          <div className="flex items-center justify-between">
                            <span className="text-[10px] font-black text-slate-900 truncate max-w-[120px]">{c.userName}</span>
                            <span className="text-[8px] font-bold text-slate-400">{new Date(c.createdAt).toLocaleDateString()}</span>
                          </div>
                          <div className="p-3 bg-white rounded-xl border border-slate-100 shadow-sm text-[11px] leading-relaxed text-slate-700">{c.text}</div>
                        </div>
                      ))}
                    </div>
                  </ScrollArea>
                  <div className="p-4 bg-white border-t space-y-3 shrink-0">
                    <Textarea 
                      ...
                      className="min-h-[80px] rounded-xl text-xs bg-slate-50 border-none shadow-inner"
                    />
                    <Button className="w-full h-9 rounded-xl font-bold text-[10px] gap-2" onClick={handleAddComment} disabled={isCommenting || !commentText.trim()}>
